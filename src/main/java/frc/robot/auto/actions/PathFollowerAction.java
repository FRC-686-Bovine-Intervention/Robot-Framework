...
public class PathFollowerAction implements Action
{
  ...
  public PathFollower getDriveController()
  {
    return driveCtrl;
  }
  ...
  {
    System.out
        .println("PathFollowerAction.start(), pose = " + RobotState.getInstance().getLatestFieldToVehicle().toString());
    driveCtrl.start();
  }
  ...
  {
    System.out
        .println("PathFollowerAction.done(),  pose = " + RobotState.getInstance().getLatestFieldToVehicle().toString());
    // cleanup code, if any
    driveCtrl.done();
  }
