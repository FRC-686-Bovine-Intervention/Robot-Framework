...
/**
 * Implements Team 254's Cheesy Drive. "Cheesy Drive" simply means that the
 * "turning" stick controls the curvature of the robot's path rather than its
 * rate of heading change. This helps make the robot more controllable at high
...
public class CheesyTriggerDriveJoystick extends JoystickControlsBase
{
    ...
    {
        double throttle = mStick.getRawAxis(Constants.kXboxRTriggerAxis)
                - mStick.getRawAxis(Constants.kXboxLTriggerAxis);
        ...
        else
        {
            ...
            if (mQuickStopAccumulator > 1) mQuickStopAccumulator -= 1;
            else if (mQuickStopAccumulator < -1) mQuickStopAccumulator += 1;
            else mQuickStopAccumulator = 0.0;
        }
