...
public class Kinematics
{
    ...
    public static class WheelSpeed
    {
        ...
        public void limit(double _limit)
        {
            ...
            if (maxSpeed > _limit) scale(_limit / maxSpeed);
        }
    }
    ...
    // can also get linear distance and change in heading from change in left/right
    // wheel distance over a period of time
    ...
    {
        double linearSpeed = (_lSpeed + _rSpeed) / 2; // linear speed of center of robot is the average of the left and
                                                      // right
        double dSpeed = (_rSpeed - _lSpeed) / 2; // differential speed of wheels (positive: turning to left, increasing
                                                 // theta)
        double angularSpeed = dSpeed * 2 * DriveLoop.kTrackScrubFactor / DriveLoop.kTrackEffectiveDiameter; // angular
                                                                                                            // velocity
                                                                                                            // (in
                                                                                                            // rad/sec)
                                                                                                            // due to
    ...
    /**
     * Forward kinematics using encoders and explicitly measured rotation (ie. from
     * gyro)
     */
    ...
    public static Pose travelArc(Pose _initialPose, LinearAngularSpeed _speed)
    {
        ...
        if (Math.abs(dTheta) > 1e-9) L = 2 * D * Math.sin(dTheta / 2) / dTheta; // chord-length given change in heading
    ...
    /*
     * Calculate left/right wheel speeds that will give desired robot speed and
     * change of heading
     * 
     ...
     * 
     * Vl = w*(R-L/2) = w*R - w*L/2 = V - w*L/2 = V - dV Vr = w*(R+L/2) = w*R +
     * w*L/2 = V + w*L/2 = V + dV
     * 
     * dV = w*L/2 = V/R*L/2 = V*K*L/2
     * 
     * where V = w*R is the linear velocity of the robot and w = V/R = V*K, is the
     * angular velocity of the robot and K=1/R is the curvature of an arc with
     * radius R
     */
