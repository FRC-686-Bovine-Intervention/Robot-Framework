...
/**
 * A class that stores the pose an object The pose consists of it's position:
 * (x,y) coordinates and it's orientation: the heading The reference coordinate
 * system for the pose is not defined in this class. The caller must keep track
 * of it.
 */
public class Pose implements Interpolable<Pose>
{
    ...
    public Vector2d getHeadingUnitVector()
    {
    ...
    // that gives the position and heading of the first coordinate system with
    // respect to that's coordinate system
    // for example: the pose of the camera on the robot is found relative to the
    // field
    // this would be the pose of the camera with respect to the robot's center of
