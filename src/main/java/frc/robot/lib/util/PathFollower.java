...
{
	/*
	 * Controller can be started in PATH_FOLLOWING or VISION state, as set by
	 * _initialState in constructor If in PATH_FOLLOWING state, it will transition
	 * to VISION state when the robot locates a vision target. It will only search
	 * for vision targets on vision-enabled path segments (set in
	 * PathSegmentOptions) It will not transition back to PATH_FOLLOWING. Start a
	 * new Action to continue following a path moving after this Action has
	 * completed.
	 */
	public enum PathVisionState
	{
		PATH_FOLLOWING, VISION
	};
	...
	public WheelSpeed pathVisionDrive(double _currentTime, Pose _currentPose, Pose _previousPose,
			double _imageTimestamp, double _normalizedTargetX, double _normalizedTargetWidth)
	{
		...
		if (visionEnabledSegment) visionDrive(_currentTime, _currentPose, _previousPose, _imageTimestamp,
				_normalizedTargetX, _normalizedTargetWidth);
		else pathDrive(_currentTime, _currentPose);
	...
	public WheelSpeed getWheelVelocity()
	{
		return getWheelVelocity();
	}

	public Path getPath()
	{
		return path;
	} // warning: not returning a defensive copy
	...
	public PathVisionState getPathVisionState()
	{
		return state;
	}
	...
	private void pathDrive(double _currentTime, Pose _currentPose)
	{
		...
		if (path.getReverseDirection()) headingToTarget -= Math.PI; // flip robot around

		curvature = 2 * Math.sin(headingToTarget) / lookaheadDist;
	}

	// drive towards vision target (or follow path if no target acquired)
	public void visionDrive(double _currentTime, Pose _currentPose, Pose _previousPose, double _imageTimestamp,
			double _normalizedTargetX, double _normalizedTargetWidth)
	{
		ledRelay.set(Relay.Value.kOn); // turn on LEDs during Vision-enabled segments
		distanceToTargetInches = Double.MAX_VALUE;

		// If we get a valid message from the Vision co-processor, update our estimate
		...
		if (_normalizedTargetWidth > 0)
		{
			...
			prevDistanceToTargetInches = Constants.kTargetWidthInches
					/ (2.0 * _normalizedTargetWidth * Constants.kTangentCameraHalfFOV);
			...
			if (prevDistanceToTargetInches < Constants.kVisionMaxDistanceInches) // ignore bogus targets
			{
				...
				{
					avgTargetLocation = avgTargetLocation.expAverage(targetLocation,
							Constants.kTargetLocationFilterConstant);
				}
			}
		}
	...
	public void speedControl(double _currentTime, double _remainingDistance, double _finalSpeed, double _maxSpeed,
			double _maxAccel)
	{
		// ---------------------------------------------------
		// Apply speed control
		...
		if (accel > _maxAccel) speed = prevSpeed + _maxAccel * dt;
		else if (accel < -_maxAccel) speed = prevSpeed - _maxAccel * dt;
		...
		if (speed > maxBrakingSpeed) speed = maxBrakingSpeed;
	...
	public boolean isFinished()
	{
		...
		else done = (remainingDistance <= Constants.kVisionCompletionTolerance);
