...
public class TalonSRXChecker
{
    ...
    public static boolean CheckTalons(Subsystem subsystem, ArrayList<TalonSRXConfig> talonsToCheck,
            CheckerConfig checkerConfig)
    {
        ...
        for (TalonSRXConfig config : talonsToCheck)
        {
            ...
            {
                System.out.println(
                        config.mName + " has failed current floor check vs " + checkerConfig.mCurrentFloor + "!!");
                failure = true;
            }
            if (checkerConfig.mRPMSupplier != null)
            {
