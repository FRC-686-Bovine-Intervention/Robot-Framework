 ...
 * instance of this class will cause communications with the sensor to being.
 * All communications with the sensor occur in a separate thread from your robot
 * code to avoid blocking the main robot program execution.
 * 
 * Example: private static BNO055 imu;
 * 
 * public Robot() { imu = BNO055.getInstance(BNO055.opmode_t.OPERATION_MODE_IMU,
 * BNO055.vector_type_t.VECTOR_EULER); }
 * 
 * Example 2: When you want gyro angle readings:
 * 
 * public void robotInit() { bno055 = BNO055.getInstance(I2C.Port.kOnboard); }
 * 
 * 
 * 
 ...
 * using the getVector() method. See this method definition for usage info.
 * 
...
public class BNO055 extends GyroBase
{
	...
	// The maximum time it took for an I2C read of data (should be less than .005
	// seconds)
	...
	// Number of times read took more than READ_TOO_LONG_THRESHOLD seconds
	// (hopefully 0)
	...
	public enum opmode_t
	{
		...
		OPERATION_MODE_ACCONLY(0X01), OPERATION_MODE_MAGONLY(0X02), OPERATION_MODE_GYROONLY(
				0X03), OPERATION_MODE_ACCMAG(0X04), OPERATION_MODE_ACCGYRO(
						0X05), OPERATION_MODE_MAGGYRO(0X06), OPERATION_MODE_AMG(0X07), OPERATION_MODE_IMU(
	 ...
	 *
	 * @param port
	 *                    the physical port the sensor is plugged into on the
	 *                    roboRio
	 * @param address
	...
	/**
	 * Get an instance of the sensor configured for IMU mode and EULER format for
	 * the purpose of reading gyro angles (assumes sensor has default I2C address).
	 * 
	 * @param port
	 ...
	 * @return the instantiated BNO055 object
	 */
	public static GyroBase getInstance(opmode_t mode, vector_type_t vectorType, I2C.Port port, byte address)
	{
		if (instance == null)
		{
	...
	private void update()
	{
		...
		if (!initialized)
		{
			// System.out.println("State: " + state + ". curr: " + currentTime + ", next: "
			// + nextTime);
			...
			case 1:
				if (currentTime >= nextTime)
				{
					...
					nextTime = Timer.getFPGATimestamp() + 0.050; // grabbing time before setMode, since setMode has it's
																	// own delay
			...
			case 5:
				...
				if (currentTime >= nextTime)
				{
					nextTime = Timer.getFPGATimestamp() + 0.050; // grabbing time before setCalibrationOffsets, since
																	// setCalibrationOffsets has it's own delay
			...
			case 9:
				...
				if (currentTime >= nextTime)
				{
					nextTime = Timer.getFPGATimestamp() + 1.050; // grabbing time before setMode, since setMode has it's
																	// own delay
	...
	public SystemStatus getSystemStatus()
	{
		 ...
		 * 1 = System Error 2 = Initializing Peripherals 3 = System Initialization 4 =
	 ...
	 * 
	 * @param verbosity
	 *                      Controls how much stuff to show (-1 nothing, 0 add
	 *                      heading, ... 9 adds everything).
	 */
