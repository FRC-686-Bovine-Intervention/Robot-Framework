...
import frc.robot.subsystems.TestDriveCommand;
import frc.robot.subsystems.TestDriveStatus;
...
public class TestDriveLoop extends LoopBase
{
 	// singleton class
	 private static TestDriveLoop instance = null;
	 public static TestDriveLoop getInstance(){if (instance == null) {instance = new TestDriveLoop();}return instance;}
	 
    private static TestDrive drive;
	private static GyroBase gyro;
    private TestDriveStatus driveState;
   ...
   public static double kFullThrottleEncoderUnitsPer100ms = 2900; 
	...
	private TestDriveLoop() 
	{
		drive = TestDrive.getInstance();
		driveState = TestDriveStatus.getInstance();
		 ...
		 *****************************************************************/
		TestDriveCommand neutralCmd = TestDriveCommand.COAST();
	...
	{
		drive.setCommand(TestDriveCommand.COAST());		// override any incoming commands 
		sendCommands();
	}
	...
	private void getStatus()
	{
		synchronized (driveState)	// lock DriveState until we update it, so that objects reading DriveState don't get partial updates	
		{
			...
			driveState.setNeutralMode( TestDriveCommand.getNeutralMode() );
	...
	{
		TestDriveCommand newCmd = drive.getCommand();
		
		// Watchdog timer  
		double currentTime = Timer.getFPGATimestamp();

		if (!drive.Enabled) newCmd = TestDriveCommand.COAST();
	...
	private void setControlMode(TestDriveCommand newCmd)
    {
	...
	private void setNeutralMode(TestDriveCommand newCmd)
	{
		NeutralMode newNeutral = TestDriveCommand.getNeutralMode();
	...
	private void setMotors(TestDriveCommand newCmd)
    {
	...
	private void resetEncoders(TestDriveCommand newCmd)
	{
